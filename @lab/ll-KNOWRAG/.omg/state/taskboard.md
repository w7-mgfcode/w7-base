...
| **T10** | Catalog UI + detail view + MCP + E2E dogfood | omg-editor + omg-verifier | T8, T9 | ❌ | All checklist items pass; `agent-browser` clean; dogfood report 0 Critical/High | ✅ verified (UI catalog smoke now in `verify.sh` as `ui_catalog.proxy_returns`, #51) |
