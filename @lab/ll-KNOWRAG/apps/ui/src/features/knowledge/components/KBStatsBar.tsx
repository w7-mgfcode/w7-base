...
function StatCell({ value, label }: { value: string | number; label: string }) {
  return (
    <div className="flex flex-col items-center px-5 border-r border-border last:border-r-0">
...
export function KBStatsBar({ activeCrawlCount, onActiveCrawlClick }: KBStatsBarProps) {
  ...
  return (
    <div className="flex items-center bg-bg-secondary border-b border-border px-4 h-12">
      ...
      {activeCrawlCount > 0 && (
        <button
          ...
          className="flex items-center gap-2 text-xs text-warning cursor-pointer hover:opacity-80"
        >
